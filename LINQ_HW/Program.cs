...
class Program
{
    ...
    {
        ////1
        //var SaitBorclu = debtors.Where(d => "AEIOUaeiou".Contains(d.FullName.FirstOrDefault())).ToList();
        //SaitBorclu.ForEach(d => Console.WriteLine(d));

        ////2
        //var DomeninBorclu = debtors.Where(d => d.Email.EndsWith("rhyta.com") || d.Email.EndsWith("dayrep.com")).ToList();
        //DomeninBorclu.ForEach(d => Console.WriteLine(d));

        ////3
        //var Yas26_32Borclu = debtors.Where(d => d.BirthDay.Year <= 1997 && d.BirthDay.Year >= 1987).ToList();
        //Yas26_32Borclu.ForEach(d => Console.WriteLine(d));

        ////4
        //var Borc5000Borclu =  debtors.Where(d => d.Debt < 5000).ToList();
        //Borc5000Borclu.ForEach(d => Console.WriteLine(d));

        //// 5
        //var AdTelBorclu = debtors.Where(d => d.FullName.Length >= 18 && d.Phone.Count(c => c == '7') >= 2).ToList();
        // AdTelBorclu.ForEach(d => Console.WriteLine(d));

        ////6
        //var QisBorclu = debtors.Where(d => d.BirthDay.Month == 12 || d.BirthDay.Month == 1 || d.BirthDay.Month == 2).OrderBy(d => d.Address.Substring(d.Address.Length - 5)).ToList();
        //QisBorclu.ForEach(d => Console.WriteLine(d));
        ...
        ////8
        //var PopDomeinBorclu = debtors.GroupBy(d => d.Email.Split('@')[1]).OrderByDescending(g => g.Count()).First().ToList();
        //PopDomeinBorclu.ForEach(Console.WriteLine);

        ////9
        //var EnCoxBorcluDogulanIl = debtors.GroupBy(d => d.BirthDay.Year).OrderByDescending(g => g.Count()).First();
        //Console.WriteLine(EnCoxBorcluDogulanIl.Key);


        ////10
        //var Ilk5Borclu =  debtors.OrderByDescending(d => d.Debt).Take(5).ToList();
        //Ilk5Borclu.ForEach(d => Console.WriteLine(d));

        ////11
        //Console.WriteLine(debtors.Sum(d => d.Debt));

        ////12
        //var WW2SagBorclu = debtors.Where(d => d.BirthDay.Year < 1945).ToList();
        //WW2SagBorclu.ForEach(d => Console.WriteLine(d));

        ////13
        //var SSRİDogulanBorclu = debtors.Where(d => d.BirthDay.Year > 1921).ToList();
        //SSRİDogulanBorclu.ForEach(d => Console.WriteLine(d));


        ////14
        //var Dollar500Borclu = debtors.Where(d => Math.Abs(d.BirthDay.Month - DateTime.Now.Month) * 500 >= d.Debt).ToList();
        //Dollar500Borclu.ForEach(d => Console.WriteLine(d));

        ////15
        //var CoxUzun = debtors.Where(d => (d.FullName.Contains('s') || d.FullName.Contains('S')) && (d.FullName.Contains('m') || d.FullName.Contains('M')) && (d.FullName.Contains('i') || d.FullName.Contains('I')) && (d.FullName.Contains('L') || d.FullName.Contains('l')) && (d.FullName.Contains('e') || d.FullName.Contains('E'))).ToList();
        //CoxUzun.ForEach(d => Console.WriteLine(d));
